...
const Chatbot = () => {
  ...
  return (
    <div className="fixed bottom-6 right-1">
      <div className={`flex mb-[5vh]  flex-col bg-[${bg}] backdrop-blur-2xl w-[20vw] h-[50vh] z-10 opacity-0 rounded-lg transition-opacity duration-500 ${isActive ? 'opacity-100' : ''}`}>
        ...
        <div className="flex-1 overflow-y-auto  px-2 py-2 text-[2vh] my-0">
          <div>
            {messages.slice().map((item, index) => (
               ...
               <div className='flex'>
               {item.name === "Sam" ? <></> : (<>
                      <div className='w-[7vw] h-[2vh]'>

                      </div>
               </>)}
               <div key={index} className={`bg-white/10 rounded-md w-[12vw]  pb-1 pl-2 mb-1 my-0 ${item.name === "Sam" ? 'bg-primary text-white ' : 'bg-secondary'}`}>
                <div className = "text-[1vh] flex">{item.name === "Sam" ? <>
                <div className='flex pt-1 h-[3vh] w-[3vw] align-center justify-left gap-1 '>

                
                <img src = "https://i.gifer.com/origin/71/71a32055ba174b23a7279ec84d4c44db_w200.gif" className='w-[1vw] h-[1vw] rounded-lg'>
                
                </img><div className = "h-[3vh] w-[1vw] mt-[0.05vh] align-center font-bold text-[aqua] text-[1vh]">codebot</div>
                </div>
                </> : <>@User</>}</div> 
                {item.message}
              </div>
